...
class PerceptronModel(Module):
    ...
    def run(self, x):
        ...
        """
        "*** YOUR CODE HERE ***"
        return tensor([tensordot(self.w, x)])
        ...
        with no_grad():
            ...
            while iteration_count < 100:
                ...
                for batch in dataloader:
                    prediction = tensor(self.get_prediction(batch['x']))
                    if prediction != batch['label']:
                        mistake += 1
                        if self.get_prediction(batch['x']) == -1:
                            self.w += learning_rate * batch['x']
                        else:
                            self.w -= learning_rate * batch['x']
                iteration_count += 1
                if mistake == 0:
...
class DigitClassificationModel(Module):
    ...
    def __init__(self):
        ...
        output_size = 10
        "*** YOUR CODE HERE ***"
        self.layer1 = Linear(input_size, 180)
        self.layer2 = Linear(180, 30)
        self.layer3 = Linear(30, output_size)
...
class LanguageIDModel(Module):
    ...
    def run(self, xs):
        ...
        for i in range(len(xs)):
            if i == 0:
                ret = self.layer1(xs[i])
            else:
                ret = self.layer1(xs[i]) + self.layer2(ret)
            ret = relu(ret)
        ret = self.extraLayer(ret)
        return ret
...
def Convolve(input: tensor, weight: tensor):
    ...
    Output_Tensor = tensor([[0.0 for _ in range(output_w)] for _ in range(output_h)])
