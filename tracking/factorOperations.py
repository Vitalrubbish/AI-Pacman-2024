...
def joinFactors(factors: List[Factor]):
    ...
    for factor in factors:
        ...
        for var in factor.conditionedVariables():
            if var not in unconditioned and var not in conditioned:
                conditioned.append(var)
    ret = Factor(unconditioned, conditioned, domain)
    for assignment in ret.getAllPossibleAssignmentDicts():
        probability = 1
        for factor in factors:
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor: Factor, eliminationVariable: str):
        ...
        for assignment in ret.getAllPossibleAssignmentDicts():
            ...
            ret.setProbability(assignment, prob)
        return ret
