...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for i in range(X_RANGE + Y_RANGE + MAX_NOISE - 1):
        domain.append(i)
    variableDomainsDict[OBS0] = domain
    variableDomainsDict[OBS1] = domain
    domain = []
    for i in range(X_RANGE):
        for j in range(Y_RANGE):
            domain.append((i, j))
    variableDomainsDict[PAC] = domain
    variableDomainsDict[GHOST0] = domain
    variableDomainsDict[GHOST1] = domain
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        # Get all CPTs and reduce by evidence
        currentFactors = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        for eliminationFactor in eliminationOrder:
            currentFactors, joinedFactor = joinFactorsByVariable(currentFactors, eliminationFactor)
            if len(joinedFactor.unconditionedVariables()) > 1:
                joinedFactor = eliminate(joinedFactor, eliminationFactor)
                currentFactors.append(joinedFactor)
        return normalize(joinFactors(currentFactors))
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for position in self.allPositions:
            ...
            for position_t in self.allPositions:
                new_belief[position_t] += distribution[position_t] * self.beliefs[position]
...
class ParticleFilter(InferenceModule):
    ...
    def getBeliefDistribution(self):
        ...
        for position in beliefs:
            beliefs[position] /= self.numParticles
        return beliefs
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for position in beliefs:
            beliefs[position] *= self.getObservationProb(observation, pacmanPosition, position, self.getJailPosition())
        if beliefs.total() == 0:
            self.initializeUniformly(gameState)
            beliefs = self.getBeliefDistribution()
