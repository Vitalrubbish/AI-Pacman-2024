...
def sentence3() -> Expr:
    ...
    ans = []
    ans.append( PA1 % ((PA0 & (~PK0)) | ((~PA0) & PB0)) )
    ans.append( ~(PA0 & PB0))
    ans.append( PB0)
    return conjoin(ans)
...
def exactlyOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    ans = [atLeastOne(literals), atMostOne(literals)]
    return conjoin(ans)
#______________________________________________________________________________
# QUESTION 3
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for (x, y) in all_coords:
        Expr1 = PropSymbolExpr(wall_str, x, y)
        Expr2 = PropSymbolExpr(pacman_str, x, y, time = t)
        literals.append(Expr1 >> (~Expr2))
    pacphysics_sentences.append(conjoin(literals))

    literals = []
    for (x, y) in non_outer_wall_coords:
        exp = PropSymbolExpr(pacman_str, x, y, time = t)
        literals.append(exp)
    pacphysics_sentences.append(exactlyOne(literals))

    N = PropSymbolExpr('North', time = t)
    E = PropSymbolExpr('East', time = t)
    S = PropSymbolExpr('South', time = t)
    W = PropSymbolExpr('West', time = t)
    pacphysics_sentences.append(atLeastOne([N, S, E, W])) #方向的顺序非常重要
    pacphysics_sentences.append(atMostOne([N, S, E, W]))
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(0, 51):
        ...
        S = PropSymbolExpr('South', time=t)
        W = PropSymbolExpr('West', time=t)
        KB.append(atLeastOne([N, S, E, W]))  # 方向的顺序非常重要
        KB.append(atMostOne([N, S, E, W]))
        for (x, y) in non_wall_coords:
...
def foodLogicPlan(problem) -> List:
    ...
    for (fx, fy) in food:
        if (fx, fy) != (x0, y0):
            foodExprs.append(PropSymbolExpr(food_str, fx, fy, time = 0))
        else:
            foodExprs.append(~PropSymbolExpr(food_str, fx, fy, time=0))
    KB.append(conjoin(foodExprs))
    for t in range(0, 51):
        literals = []
        for (x, y) in non_wall_coords:
            exp = PropSymbolExpr(pacman_str, x, y, time = t)
        ...
        for (fx, fy) in food:
            if t > 0:
                KB.append(~PropSymbolExpr(food_str, fx, fy, time = t) %
                          (~PropSymbolExpr(food_str, fx, fy, time = t - 1) |
                           (PropSymbolExpr(food_str, fx, fy, time = t - 1) &
                            PropSymbolExpr(pacman_str, fx, fy, time = t))))
        goalExpr = []
        for (fx, fy) in food:
            goalExpr.append(~PropSymbolExpr(food_str, fx, fy, time = t))
        goal = conjoin(goalExpr)
        model = findModel(conjoin(KB + [goal]))
        if model:
            return extractActionSequence(model, actions)
        N = PropSymbolExpr('North', time=t)
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        possible_locations = []
        for (x, y) in non_outer_wall_coords:
            goal = PropSymbolExpr(pacman_str, x, y, time = t)
            if findModel(conjoin(KB + [goal])):
                possible_locations.append((x, y))
            if entails(conjoin(KB), goal):
                KB.append(goal)
            if entails(conjoin(KB), ~goal):
                KB.append(~goal)
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        KB.append(fourBitPerceptRules(t, agent.getPercepts()))
        for (x, y) in non_outer_wall_coords:
            goal = PropSymbolExpr(wall_str, x, y)
            if entails(conjoin(KB), goal):
                known_map[x][y] = 1
                KB.append(goal)
            if entails(conjoin(KB), ~goal):
